# Legal Prompt Library

A curated collection of practical AI prompts for attorneys — organized by task, copy-paste ready, and sourced from publicly shared attorney experiences.

## How to Use

1. Find the category that matches your task
2. Copy the prompt template
3. Replace `[BRACKETED PLACEHOLDERS]` with your specifics
4. Paste into ChatGPT, Claude, or your preferred LLM
5. Always review and verify AI output before relying on it professionally

## ⚠️ Important Disclaimers

- **Verify everything.** AI can hallucinate case citations, statutes, and legal standards. Always independently verify legal authorities before relying on them.
- **Confidentiality.** Do not paste client confidential information into public AI tools. Use enterprise/private deployments with appropriate data protection agreements.
- **Jurisdiction matters.** Many prompts reference specific jurisdictions. Adapt them to your applicable law.
- **Not a substitute for judgment.** These prompts are starting points. Attorney review, supervision, and professional judgment are always required.

## Categories

| File | Topics Covered |
|------|---------------|
| [01 — Contract Drafting](01-contract-drafting.md) | First drafts, clauses, templates |
| [02 — Contract Review](02-contract-review.md) | Redlines, risk analysis, summaries |
| [03 — Legal Research](03-legal-research.md) | Research memos, case law, statutes |
| [04 — Litigation](04-litigation.md) | Motions, discovery, depositions, trial |
| [05 — Client Communications](05-client-communications.md) | Demand letters, status updates, intake |
| [06 — Data Privacy & Compliance](06-data-privacy-compliance.md) | GDPR, CCPA, data breach, DPAs |
| [07 — Corporate & Transactional](07-corporate-transactional.md) | M&A, governance, employment |
| [08 — Intellectual Property](08-ip.md) | Patents, trademarks, trade secrets |
| [09 — Legal Operations](09-legal-ops.md) | Intake, KPIs, department management |
| [10 — Prompting Frameworks](10-prompting-frameworks.md) | ABCDE, RICE, tips for better results |

## Contributing

Prompts that work well in practice are welcome. Please include:
- The prompt text
- The use case / practice area
- The AI tool it was tested on (optional)
- Any important caveats

## Sources

Prompts in this library are sourced and adapted from:
- [Ten Things Blog — "100 Practical GenAI Prompts for In-House Lawyers" by Sterling Miller](https://tenthings.blog/2025/03/31/ten-things-practical-generative-ai-prompts-for-in-house-lawyers/) — Miller is a former General Counsel at Marketo and Sabre with nearly 25 years of in-house experience
- [Clio Blog — "ChatGPT Prompts for Lawyers"](https://clio.com/blog/chat-gpt-prompts/) — published by Clio, a leading legal practice management platform
- [MyCase Blog — "ChatGPT for Lawyers"](https://mycase.com/blog/ai/chatgpt-for-lawyers/) — published by MyCase, a legal practice management platform (owned by 8am, LLC)
- [CasePeer Blog — "Top 40 ChatGPT Prompts for Lawyers"](https://casepeer.com/blog/chatgpt-prompts-for-lawyers/) — published by CasePeer, a personal injury case management platform (also owned by 8am, LLC)
- [Spellbook Legal — "ChatGPT for Lawyers: 20+ Prompts That Actually Work"](https://spellbook.legal/learn/chatgpt-for-lawyers)
- [Tavrn.ai — "Complete Guide to AI Prompting for Lawyers"](https://tavrn.ai/blog/ai-prompting-for-lawyers)
- [Leah AI (formerly ContractPodAi) — "AI Prompts for Legal Professionals"](https://leahai.com/news/ai-prompts-for-legal-professionals) — source of the ABCDE prompting framework
- [Widener University Delaware Law Library — "Generative AI & Legal Research" LibGuide](https://libguides.law.widener.edu/c.php?g=1342893&p=10038411)
- [NC Bar Association — "Prompt Engineering 101 for Lawyers" by Catherine Sanders Reach](https://www.ncbar.org/nc-lawyer/2024-08/prompt-engineering-101-for-lawyers/) — published in *North Carolina Lawyer* magazine (August 2024); source of the RICE framework
- [GitHub: TracyWang95/legal-prompts-for-gpt](https://github.com/TracyWang95/legal-prompts-for-gpt) — community-contributed open source repo (Notre Dame Law graduate)

---

*Maintained by [@Vibe-Coding-Lawyer](https://github.com/Vibe-Coding-Lawyer)*
